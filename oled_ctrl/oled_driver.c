...
int lcd_read_value(const char *filename)
{
	...
	{
#if BOXMODEL_DM8000 || BOXMODEL_DM7080 || BOXMODEL_DM7020HD || BOXMODEL_DM800SE || BOXMODEL_DM800SEV2
...
int lcd_open(const char *dev, int mode, int x_res, int y_res)
{
	...
	if (xres == 132 && yres == 64 && bpp == 8) {
		printf("DM7020HD/DM8000/DM7080/DM800SE original LCD not supported.\n", xres, yres, bpp);
		return 0;
	}
#endif
...
{
#if !BOXMODEL_DM8000 && !BOXMODEL_DM800SE && !BOXMODEL_DM800SEV2 && !BOXMODEL_E4HDULTRA && !BOXMODEL_VUULTIMO && !BOXMODEL_DM7020HD && !BOXMODEL_DM820 && !BOXMODEL_DM7080 && !BOXMODEL_DM900 && !BOXMODEL_DM920
	int tmp;
	if (mode == 0)
	{
		tmp = LCD_MODE_BIN;
